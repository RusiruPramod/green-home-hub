...
const Energy = () => {
  ...
  const energyStats = [
    ...
    { label: "Estimated Bill", value: "₹2,450", change: "-15%", positive: true },
  ];
    ...
    <div className="flex min-h-screen w-full bg-background">
      ...
      <main className="flex-1 overflow-auto">
        <header className="sticky top-0 z-10 flex h-16 items-center justify-between border-b border-border bg-background/80 px-6 backdrop-blur-lg">
          <div>
            <h1 className="text-xl font-bold text-foreground">Energy Monitoring</h1>
            <p className="text-sm text-muted-foreground">Track power consumption & efficiency</p>
          </div>
        ...
        <div className="space-y-6 p-6">
              ...
              <Card key={stat.label} className="animate-fade-up opacity-0" style={{ animationDelay: `${index * 100}ms`, animationFillMode: 'forwards' }}>
                <CardContent className="pt-6">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`alert-badge ${stat.positive ? "alert-badge-success" : "alert-badge-warning"}`}>
                      {stat.positive ? <TrendingDown className="h-3 w-3" /> : <TrendingUp className="h-3 w-3" />}
                      {stat.change}
                    </span>
                  </div>
                  <p className="mt-2 text-2xl font-bold text-foreground font-mono">{stat.value}</p>
                </CardContent>
              </Card>
            ))}
          </div>
          ...
          {/* Live Readings */}
          <div>
            <h2 className="mb-4 text-lg font-semibold text-foreground">Live Readings</h2>
            ...
            <CardContent>
              <div className="grid gap-4 sm:grid-cols-3">
                <div className="rounded-lg bg-destructive/10 p-4">
                  <p className="text-sm font-medium text-destructive">High Usage</p>
                  <p className="text-lg font-bold text-foreground">6 PM - 10 PM</p>
                  <p className="text-xs text-muted-foreground">Avg: 850W</p>
                </div>
                <div className="rounded-lg bg-warning/10 p-4">
                  <p className="text-sm font-medium text-warning">Medium Usage</p>
                  <p className="text-lg font-bold text-foreground">8 AM - 6 PM</p>
                  <p className="text-xs text-muted-foreground">Avg: 420W</p>
                </div>
                <div className="rounded-lg bg-success/10 p-4">
                  <p className="text-sm font-medium text-success">Low Usage</p>
                  <p className="text-lg font-bold text-foreground">10 PM - 8 AM</p>
                  <p className="text-xs text-muted-foreground">Avg: 120W</p>
                </div>
              </div>
            </CardContent>
          </Card>
